...
def plan_route_distribution(
    *,
    ...
    ``urm.compiler.placement.routes``; the directions are never conflated.)
...
class UrmCompiler:
    ...
    def _search_schedule(
        ...
        *,
        ...
        if not any(isinstance(op, WeightedReduce) for op in compiled.ops):
            return None
        from urm.compiler.schedule.search import CompilationSearch
                        ...
                        decision = AnchorDecision(
                            ...
                            decline=Decline(
                                ...
                                message=(
                                    ...
                                    f"the native sparse route anchor "
                                    f"{NATIVE_SPARSE_ROUTE_ANCHOR_NAME!r}"
                                ),
                            ),
                        )
    ...
    def _lower_to_anchors(
        ...
        for op in compiled.ops:
            ...
            if anchor.kind is AnchorKind.SPARSE_ROUTE_SELECTION:
                ...
                block_route = max(2, 1 << (op.spec.route_width - 1).bit_length())
                step_launch_config = {
    ...
    def _apply_override(
        ...
        # An external-adapter override must still satisfy its pinned upstream
        ...
        if override_name == sdm_fallback:
            ...
            if not upstream_status.supported:
                return AnchorDecision(
                    anchor=None,
                    decline=Decline(
                        reason_code={
                        ...
                        message=(
                            ...
                            or f"SDM fallback override declined: {upstream_status.code}"
                        ),
                    ),
                )
    ...
    def _is_interior_gather(program: SemanticProgram, op: SemanticNode) -> bool:
        from urm.ir.program import Gather as GatherOp
        from urm.ir.program import WeightedReduce
    ...
    def _request_for(  # noqa: C901 - dispatch over typed ops
        ...
        from urm.compiler.placement.locality import Locality
    ...
    def _cost_for(
        ...
        if isinstance(op, SparseRouteGeneration):
            ...
            rows = spec.parallel * spec.sequence
            score_bytes = rows * spec.score_width * dtype_bytes
            route_bytes = rows * spec.route_width * (4 + dtype_bytes)
            return CostEstimate(
                useful_flops=rows * spec.route_width,
                logical_bytes=score_bytes + route_bytes,
                physical_bytes_estimate=score_bytes + route_bytes,
                launch_count=1,
