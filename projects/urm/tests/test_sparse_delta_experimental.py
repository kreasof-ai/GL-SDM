...
from urm.experimental.sparse_delta import SparseDeltaFunction, sparse_delta


@pytest.mark.skipif(
    not torch.cuda.is_available(), reason="CUDA required for Tensor Core backend"
)
...
def test_decoupled_sparse_indexer_compatibility():
    """Verify that sparse_delta seamlessly interfaces with decoupled sparse routers (e.g. Foveal or Product-Key)."""
    device = "cuda"
    dtype = torch.bfloat16
    ...
    write_weights = torch.softmax(
        torch.randn(P, T, W, device=device, dtype=dtype), dim=-1
    )
    read_indices = torch.randint(0, S, (P, T, R), device=device, dtype=torch.int64)
    read_weights = torch.softmax(
        torch.randn(P, T, R, device=device, dtype=dtype), dim=-1
    )
...
def test_exact_gradient_alignment_with_upstream_reference(seed, dtype):
    ...
    k_idx = torch.stack(
        [torch.randperm(S, device=device)[:W].sort().values for _ in range(T)]
    )
    q_idx = torch.stack(
        [torch.randperm(S, device=device)[:R].sort().values for _ in range(T)]
    )
    ...
    out_ref = upstream_reference(
        mem_ref, k_idx, kw_ref, v_ref, b_ref, g_ref, q_idx, qw_ref
    )
    ...
    out_dual, _ = SparseDeltaFunction.apply(
        mem_dual,
        q_idx.unsqueeze(0).long(),
        qw_dual,
        k_idx.unsqueeze(0).long(),
        kw_dual,
        v_dual,
        b_dual,
        g_dual,
    ...
    for name, u_t, d_t in comparisons:
        ...
        cos_sim = torch.nn.functional.cosine_similarity(
            u_f.unsqueeze(0), d_f.unsqueeze(0)
        ).item()
        abs_diff = (u_t.float() - d_t.float()).abs().max().item()

        tol = 0.03 if dtype is torch.bfloat16 else 1e-4
        assert cos_sim > 0.9999, (
            f"{name} cosine similarity failed: {cos_sim:.6f} for seed={seed}, dtype={dtype}"
        )
        assert abs_diff <= tol, (
            f"{name} max error failed: {abs_diff:.4e} > {tol} for seed={seed}, dtype={dtype}"
        )


@pytest.mark.skipif(
    not torch.cuda.is_available(), reason="CUDA required for Triton backend"
)
def test_triton_sparse_delta_forward_and_backward():
    """Verify pure Triton Dual-Form SDM kernel forward and backward pass."""
    from urm.experimental.triton_sparse_delta import triton_sparse_delta
    ...
    out, final_m = triton_sparse_delta(
        memory,
        ri,
        q,
        write_indices=wi,
    ...
    for name, tensor in [
        ...
        assert tensor.grad is not None, (
            f"Triton kernel failed to compute gradient for {name}"
        )
        assert torch.isfinite(tensor.grad).all(), f"Non-finite gradient in {name}"


@pytest.mark.skipif(
    not torch.cuda.is_available(), reason="CUDA required for Chunked Dual-Form SDM"
)
def test_chunked_sparse_delta_forward_and_backward():
    """Verify Chunked Dual-Form SDM with C=32 on T=64 sequence."""
    from urm.experimental.sparse_delta import chunked_sparse_delta
    ...
    wi = torch.stack(
        [torch.randperm(S, device=device)[:W].sort().values for _ in range(P * T)]
    ).view(P, T, W)
    ri = torch.stack(
        [torch.randperm(S, device=device)[:R].sort().values for _ in range(P * T)]
    ).view(P, T, R)
    w = torch.rand(P, T, W, device=device, dtype=dtype, requires_grad=True)
    q = torch.rand(P, T, R, device=device, dtype=dtype, requires_grad=True)
    v = torch.randn(P, T, D, device=device, dtype=dtype, requires_grad=True)
    ...
    out, final_m = chunked_sparse_delta(
        memory,
        ri,
        q,
        write_indices=wi,
    ...
    for name, tensor in [
        ...
        assert tensor.grad is not None, (
            f"Chunked Dual-Form SDM failed to compute gradient for {name}"
        )
        assert torch.isfinite(tensor.grad).all(), f"Non-finite gradient in {name}"
