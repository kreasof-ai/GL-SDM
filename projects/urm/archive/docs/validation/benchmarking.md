...
- Original Sparse Delta Memory is an external Git checkout pinned to
  ...
  4.0 source is never vendored; see `docs/adapters/sparse-delta-memory.md`.
...
[Triton backend preparation](../runtime/triton-backend.md). It compares the transparent
...
`docs/kernels/sparse-state-mixer.md` for fixed semantic/numerical envelopes.
...
`benchmarks/pretraining_step.toml` and `docs/validation/pretraining-step.md`. Its fixed
