...
def main() -> None:
    ...
    print(f"{'case':>17} {'mode':>8} {'backend':>7} | " +
          " | ".join(f"{d.name[:22]:>22}" for d in dirs) + " | delta med")
    violations = []
    for case in CASES:
        for mode in MODES:
            key = (case, mode)
            ...
            last = {m["backend"].split("_")[0]: m for m in runs[-1][key]["measurements"]}
            for backend in ("torch", "triton"):
                b, l = base[backend], last[backend]
                row = f"{case:>17} {mode:>8} {backend:>7} | "
                for run, d in zip(runs, dirs):
                    m = next(x for x in run[key]["measurements"]
            ...
            if bc and lc:
                ...
                for artifact in bc:
                    ...
                    if lc[artifact]["max_abs_error"] > limit:
                        violations.append((key, f"{artifact}>{limit:.2e}",
            ...
            if "triton" in base and "triton" in last:
                ...
                if l["p95_ms"] > b["p95_ms"] * (1.0 + allowed):
                    violations.append((key, f"triton-p95 allowed<={allowed:.1%}",
                                       b["p95_ms"], l["p95_ms"]))
                if l["peak_allocated_bytes"] > b["peak_allocated_bytes"] * 1.02 + 2**20:
                    violations.append((key, "triton-peak",
    ...
    if violations:
        ...
        for v in violations:
            print(" ", v)
        sys.exit(1)
    print("\nNo constraint violations (correctness deltas, p95 within +3%, peak mem within +2% or 1 MiB).")
