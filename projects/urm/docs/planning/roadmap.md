...
- Still open from the baseline: K3 route-provenance/read-timing hardening, the SDM probe removal, and native K2/K3 schedules admitted through the same contract (Gate 2 — the two public native K2 anchor names currently execute the reference recurrence as a fail-closed placeholder, an explicit non-claim). `architectures/` now has external layer modules but not complete source-model applications; `inference/` and the SDM-tied `train/loop.py` still need Gate 3 work. The old 62-recipe master table and 76 kernel-slice comparisons are historical; neither certifies current model coverage. See [evidence](../validation/evidence.md).
