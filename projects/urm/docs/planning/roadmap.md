...
> **Gate 3 training harness (partial).** `train/` is now a model-agnostic ATMA-pattern harness: `data` (finewebedu shards + a synthetic stream), `model` (a pluggable-mixer `URMDecoderLM`), `registry` (the architecture→upstream map with honest capability flags — HLA has no reference kernel, TDA no decode kernel, both recorded), `optimizer` (AdamW+Muon with a coverage assertion), `harness` (the runner + the three correctness gates), and `run` (the CLI). It trains any registered public-path mixer; the **checkpoint-alignment, gradient-alignment, and KL-divergence gates pass** for the registered mixers (`dense_attention`, `gla`, `deltanet`, `forgetting_attention`), and loss decreases with the tuned AdamW+Muon rates. **Open:** the 50%-MFU @ 100M target is NOT met — the native K2 mixer is fast (10 steps in 3.5s) but the unfused surround (embedding + tied vocab-50304 head + backward) dominates at ~5% MFU, and `torch.compile` cannot trace the native Triton mixer kernel; closing the gap is a kernel-compilation/fusion task, recorded not claimed. The SDM K3 model and the complete-model sources (Samba, PAttention) remain Gate 3 work. `train/loop.py` is retained for the SDM comparison evidence.
