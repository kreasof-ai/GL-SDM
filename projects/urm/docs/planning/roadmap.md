...
## Current baseline (with Batches 1 and 2 landed)
...
All 30 composition rows (16 now + 14 k2-gated) are landed as validated external model modules. Residual blockers (cache/decode modes, router tie-policy identity for SDM, Mamba axis for Samba, compressed-cache equality for MLA, the U2 rewrite for H3/Hyena, and the per-row external-stage variants recorded in each recipe) are recorded, not claimed.
