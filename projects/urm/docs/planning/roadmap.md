...
- **Gate 4 axis admissions landed (partial, two-client gate per sub-law):** the typed **Merge** operator (A14, Batch 3) plus five generality axes admitted as typed IR/descriptor structure — A13 score/reducer (`K1ScoreLaw` channel-decay + `K1ReducerLaw` threshold/squared-sum; FoX, Wall, TDA, KATA), A2 indexed-K1 (`K1Descriptor.indexed` + `gather_indices`; NSA, Longformer, MoBA, DSA, Sparse Transformer), A8 K2 transition breadth (generalized rank-1 `erase_gate`/`write_gate`/`predict_key`/`low_rank`, ordered multi-delta `num_deltas`, and the selective diagonal-SSM `selective_scan_diag`; GDN2, RWKV-7/DPLR, IPLR, Comba, Gated DeltaProduct, Mamba-1, Mamba-2), A4 hierarchical chunks (the dyadic level-state law; Log-linear, LogLinearMamba2), and the UT causal triangular transform (DeltaFormer strict value correction, PaTH Householder operand correction). Each sub-law was verified against its pinned source with ≥2 independent clients where the sub-law has them; single-client sub-laws (Parallax, POLAR/Foveal polar, Momentum, Oja, HGRN, RWKV-4/6, Mamba-3, FwPKM, BDH) stay external/reference-only per the two-client gate and are recorded, not claimed. 54 architecture recipes are validated.
