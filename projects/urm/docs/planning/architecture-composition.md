...
| 005 NSA | `E[compress/index/route gates] ; (U1.S[compressed] || U1.S[local] || U1.S[selected routes]) ; E[gated weighted branch merge]` | `B[exact branch equations]`, `B[indexed_K1_schedule]`, `B[route-and-compression cost]`; the source merge is a gated weighted sum (per-branch gates `g_slc/g_cmp/g_swa`), not an unweighted merge |
...
| 010 PaTH | `E[QKV,w/β/g,short convolution] ; B[typed causal triangular transform] ; U1.S(transformed operands, cumulative-log-gate score bias) ; E[output]` | Triangular operator VJP/traffic; no projection-only equivalence; the final score also adds a FoX-style cumulative log-gate bias, so this row additionally depends on the 008 A13 score-modulation axis |
...
| 012 Parallax | `E[QKV/secondary query r] ; B[K1 shared-softmax multi-statistic reducer] ; E[correction/output]` | Prove `E[v]*(1+E[r·k])-E[(r·k)v]` and reducer VJP; the source computes all statistics under one shared max/denominator, so a repeated-K1-pass fallback cannot reuse the current U1.S contract unchanged — any multi-pass plan must expose shared row statistics explicitly |
| 013 DeltaFormer | `E[QKV/β] ; B[strict-causal triangular value correction] ; U1.S(corrected V, clamped-denominator policy) ; E[output]` | Preserve strict-past correction and gradients; no ordinary attention-only claim; the source softmax clamps the denominator (`clamp_min`), so a fully-masked row yields all-zero probabilities — this edge policy is part of the descriptor |
...
| 016 Lightning | `E[projections/layer-index decay schedule] ; U2.A(static per-head scalar decay) ; E[output]` | Exact layer-indexed schedule of the per-head scalar decay, state continuation, `B[K2_graph_execution]` |
| 017 RetNet | `E[projections/rotary] ; parallel U2.A(static per-head scalar decay γ_h) ; E[head combine/norm]` | Verified: decay is a fixed per-head constant (γ_h = 1 − 2^(−5−h)) with a static causal mask, not data-dependent; independent head states and source combine, `B[K2_graph_execution]` |
| 018 Simple GLA | `E[QKV/head-gate projection] ; U2.A(data-dependent per-head scalar gate, supplied as operand) ; E[output]` | Verified: the gate is time-varying (logsigmoid of a learned projection of x_t) at head scope and enters U2.A as an operand — data-dependent per-token gates are descriptor operands, not a new axis; gate VJP and low-precision chunk path, `B[K2_graph_execution]` |
...
| ID | External call graph, in execution order | Mandatory blocker before full row claim |
|---|---|---|
| 047 SDM | `E[product-key feature projections] ; R.PK ; U3.D(certified route result) ; E[output/state ownership]` | Verified against source: the router matches `R.PK` structurally (pairwise-additive factor scores, per-factor top-k, `idx1*half+idx2` addresses, ascending sort) but diverges on two recorded points — tie policy is backend-dependent `torch.topk` rather than the mandated HIGHEST_ADDRESS, and route weights use a scaled softmax with configurable scale rather than frozen SOFTMAX. An exact `R.PK` claim requires closing those two gaps in the descriptor. Route provenance, overlap VJP, collision/read timing and full router cost; no SDM selector in core |
| 048 ABC | `E[QKV/slot gates] ; U2.A(stage-1 slot summary, channel decay) ; E[slot softmax] ; U2.A(stage-2 value summary, channel decay) ; E[output]` | Verified: both stages are additive U2.A with channel decay — no retrieval-correction term appears; gates are log-cumsum-exp-derived. Blockers: `B[K2_graph_execution]`, the interstage coupling/state ABI with both VJPs, and `B[channel-decay native envelope]`; the two calls cannot be replaced by one generic K2 by name |
| 049 GSA | `E[QKV/slot gates] ; U2.A(stage-1 slot summary, channel decay) ; E[slot softmax] ; U2.A(stage-2 value summary, channel decay) ; E[output]` | Verified: same two-stage additive U2.A structure as ABC; differs only in gate provenance (logsigmoid/8 with s=1−exp(g)). Blockers: `B[K2_graph_execution]`, the interstage coupling/state ABI with both VJPs, and `B[channel-decay native envelope]`; compare ABC/GSA gate semantics before any shared descriptor |
...
| 053 Samba | `E[config-derived ordered Block list] ; each Block = E[norm] ; selected mixer call(s): Mamba→B[A8 selective-SSM transition], attention→U1.S, retention→U2.A, or GLA→U2.A ; E[residual/optional MLP]` | Preserve the source's **two** arrangement modes: mixer selection per layer, or `mamba_swa_mlp` running Mamba then attention in one Block. Verified: the Mamba branch is the selective-SSM law (per-token diagonal gate, input-dependent but state-independent coefficients) — an A8 axis, not U2.A — so every config containing Mamba layers is gated on that axis admission, not merely `B[K2_graph_execution]`. Record exact config, order and both cache types; `samba_attention_core` is only one attention fragment |
...
| 057 Pattention | `E[construct/reparameterize parameter tokens] ; U1.S(parameter domain,source scale,source normalizer) ; E[GELU/L2/output/model replacement]` | `B[K1 parameter-domain ABI]`, parameter-token gradients and complete replaced layers; verified: the source reducer is a parameter-domain nonlinear normalizer (default exp + L1-normalize × count, with gelu_l2_norm variants) — transcribe it exactly rather than assuming vanilla softmax; no claim that one attention contraction equals MLP or MoE |
...
| 063 BDH | `E[encoder/value projections/RoPE] ; B[K2 read-timing descriptor: strict-past additive read-before-write] ; E[norm/MLP/dropout/residual]` | Exact strict-past timing; U2.A is after-update and cannot be used unchanged. The resolution path is a closed read-timing field on the K2 descriptor (before-update read) — an axis candidate, not a per-model branch |
...
| 065 Foveal | `E[geometric/local/remote route selection] ; B[K1 indexed polar reducer] ; E[projections/count/output]` | Reuse 064 reducer plus exact route ABI; include selection cost and gradients. Also depends on `B[indexed_K1_schedule]`: the reducer traverses only the local ∪ remote route set, and a dense masked implementation is a correctness oracle only |
...
| 068 TDA | `E[Q1/K1,Q2/K2,V,β/threshold/λ] ; (B[K1 threshold-ReLU-power reduction]_1 || same_2) ; E[weighted difference/output]` | Exact source normalization and threshold gradient; U1.S softmax is invalid. Verified structure: the threshold-ReLU-power reducer is the A13 score/reducer axis, and the two-branch weighted difference is the 067 Diff combinator applied to that reducer — both must be admitted (descriptor + references) before any native branch |
...
|---|---|---|
| Indexed softmax traversal and route ABI | 005/006/007/071/072; 065 as the same traversal with a non-softmax reducer; 066 as a different compressed/local graph | Exact route/visibility semantics, safe provenance, gradients, real sparse work and route-inclusive cost |
...
| Channel-decayed additive/delta state | 019/024/028/036/048/049 and 027 as a distinct update | Source gate shapes, read order, state/gradient equality; efficient long prefill |
