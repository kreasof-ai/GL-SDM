...
| `backends/triton/k2/{nonlinear,inner_state,backward}.py` | Source-named RWKV/Mamba/TTT/Titans/Hyena implementations and autograd wrappers live under K2; FFT convolution uses `torch.fft`. | Several are distinct equations or compositions, not mere tuning variants of the canonical delta recurrence. Renaming functions alone would not unify them. |
...
| `backends/triton/k2/{matrix,diagonal,nonlinear,inner_state,backward,second_order}.py` | Retain only axis-qualified native bodies and VJPs. Move architecture-named wrappers and single-use implementations to recipes/comparators until they satisfy the branch admission rule. Split FFT convolution into a reusable typed operator/provider when its independent consumers are demonstrated. |
